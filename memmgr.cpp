...
void  operator  delete(void  *pointer) _NO_EXCEPT_TRUE
{
	...
	if(pointer == NULL)
	{
		MEMMGR_LOG_WARNING(source_file,  source_line,"delete: Try to deallocate NULL pointer!");
		return;
	}
	...
	{
		MEMMGR_LOG_ERROR(source_file,source_line,"delete: Try to deallocate a pointer with CRC error. Either is a corrupted pointer or not managed pointer!");
		return;
	}
